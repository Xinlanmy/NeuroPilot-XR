...
- 2.3 视觉统一：三个房间的 HUD 卡片改成与导航面板同款（`rounded` 圆角底 + 深蓝卡片色 + 青色描边），房间内的“返回模式选择 / 重新训练”按钮修复了贴图引用悬空导致渲染成白块的问题（房间按钮改为每轮重新解析贴图，与导航一致）；SSVEP 目标球改为无镜面高光的磨砂材质。**刺激的明暗颜色、频率、占空比与时相一律未改。** 房间仍**不开启实时阴影**：2.3.0 曾开主光软阴影，真机上三个训练房掉到约 4 fps、转头成“幻灯片”（对照导航场景 98–108 fps，2.2.1 无此现象），已回退详见 [2.3.0 发布说明](ReleaseNotes_v2.3.0.md)。
