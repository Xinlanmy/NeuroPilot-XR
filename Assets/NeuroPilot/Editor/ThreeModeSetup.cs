...
namespace NeuroPilotXR.Editor
{
    public static class ThreeModeSetup
    {
        ...
        public static void Apply()
        {
            ...
            // All three rooms get the presentation pass. The eye/multi rooms are copies of TrainingRoom and
            // only inherited the HUD at copy time, so without this they drift apart — that is exactly how
            // their buttons ended up pointing at a rebuilt-away ButtonGradient sub-asset and rendered blank.
            PresentRoom(TrainingRoomIntegration.ScenePath, TrainingMode.SingleTarget);
            PresentRoom(EyePath, TrainingMode.EyeTracking);
            PresentRoom(MultiPath, TrainingMode.MultiTarget);
            PrepareNavigation();
        ...
        private static void PrepareRoom(string path, TrainingMode mode)
        {
            ...
            if (practice == null)
            {
                ...
                if (mat == null)
                {
                    ...
                    mat.color = new Color(.04f, .8f, .95f); mat.SetFloat("_Smoothness", .12f);
                    // Matte on purpose: a specular highlight keeps a constant luminance while the SSVEP
                    // on/off state modulates everything else, so gloss only dilutes the flicker contrast.
        ...
        /// <summary>
        /// Idempotent per-room presentation pass: rebuild the HUD in the shared style, relabel the
        /// per-mode caption, and re-point the room buttons at the current ButtonGradient sprite.
        /// </summary>
        private static void PresentRoom(string path, TrainingMode mode)
        {
            var scene = EditorSceneManager.OpenScene(path);
            var hud = UnityEngine.Object.FindObjectsOfType<TrainingHUD>(true).SingleOrDefault();
            var rig = UnityEngine.Object.FindObjectOfType<XROrigin>();
            if (hud == null || rig == null) throw new InvalidOperationException("Training HUD or XR rig missing in " + path);
            TrainingRoomPresentation.Prepare(hud, rig);
            var caption = hud.accuracyText.transform.parent.Find("Caption").GetComponent<UnityEngine.UI.Text>();
            caption.text = mode == TrainingMode.EyeTracking ? "注视占比" : "命中率";
            EditorUtility.SetDirty(caption);
            foreach (string name in new[] { "ReturnToModesButton", "RestartPracticeButton" })
            {
                var button = hud.transform.Find(name);
                if (button == null) continue;
                // Re-resolved every run on purpose: the sprite asset is regenerated by the project builder,
                // and a sprite reference left over from a previous generation renders as a blank white box.
                var image = button.GetComponent<Image>();
                image.sprite = buttonSprite;
                EditorUtility.SetDirty(image);
                var target = button.GetComponent<Button>();
                var colors = target.colors;
                colors.disabledColor = new Color(.62f, .68f, .76f, .85f);
