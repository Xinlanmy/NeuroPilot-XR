...
namespace NeuroPilotXR.Editor
{
    ...
    public static class ThreeModeVerification
    {
        ...
        private static IEnumerator<float> Verify()
        {
            ...
            eye.TestGaze(true, ray, 3); eye.TestGaze(false, default, .1f);
            Require(eye.DwellProgress >= .5f, "Brief tracking loss discarded the whole fixation");
            ...
            checks.Add("Visual room: four colored balls; real-eye readiness; blue dwell 5s; wrong color drains, brief eye loss tolerated, sustained loss clears, off-target adds nothing; timer pauses, no EEG confirmation; reward and replacement.");
            Click("ReturnToModesButton"); while (SceneManager.GetActiveScene().name != "NeuroPilotNavigation") yield return .1f;
        ...
        private static void Capture(string name) => TrainingRoomVerification.Capture(name + "_v2.0.png");
