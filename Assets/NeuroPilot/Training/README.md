...
| `FusionLink.cs` | WebSocket 客户端（System.Net.WebSockets，零依赖）：契约信封、1s ping 心跳、断线自动重连、离线静默降级 |
| `FusionJson.cs` | 极简扁平 JSON 解析（下行命令专用，零依赖） |
...
- Python 下行 `command_fire` → EegHitSource.NotifyHit → 命中；`command_auto` → 兜底自动完成不计分；`command_reset` → 重开
